...
- [x] **Server performance pass (v1.2.0)** — Float32Array-native radix-2 FFT replacing fft.js (eliminates `number[]` hot path, enables V8 typed-array JIT); async `deflateRaw` offloads zlib to libuv thread pool (was blocking event loop up to 150ms/sec); NCO lookup tables replace `Math.sin/cos` in FmStereoDemod PLL (480K calls/sec eliminated) and RdsDecoder (240K calls/sec eliminated); `fastAtan2` polynomial approximation replaces `Math.atan2` in FM discriminator (~4–6× faster); `SimpleFir` switched from Float64Array to Float32Array (halved memory bandwidth on 24.5M iter/sec MAC loop, enables SIMD); `pendingFrame` pre-allocated in FftProcessor (eliminates 7.5 MB/sec GC at N=65536); `CQuamStereoDemod` pre-allocates stereo output buffers (eliminates 750 KB/sec GC); `ImaAdpcmEncoder.encode()` accepts optional output buffer (zero-alloc on IQ hot path); module-level singleton encoder in `encodeFftAdpcm` (no `new ImaAdpcmEncoder` per frame); `generateSignalSample` inlined (eliminates 20K `{i,q}` objects per 33ms in demo mode).
- [x] **Audio pop/click fixes (v1.2.0)** — IQ extraction loop moved before first `await` in `_handleIqDataAsync` and defensive `Buffer.from(rawData)` copy added (Node.js stream buffer reuse was corrupting IQ data after async deflate resumed); async deflate computation moved outside client iterator loop (safe Map iteration); `iqAccumBuffer` zero-filled on tune command (stale frequency data was bleeding into next chunk); jitter buffer underrun threshold changed from single-underrun to 3-consecutive (single late IQ chunk no longer causes 150ms silence gap/pop); Opus `pcmBufferPos` accumulation loop now uses snapshotted `channels` local (mid-call stereo↔mono switch no longer corrupts frame alignment); dead `decimCounterR` removed from `FmStereoDemod`.
